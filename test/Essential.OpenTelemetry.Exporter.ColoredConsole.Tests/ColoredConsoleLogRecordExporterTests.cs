...
public class CompactLogRecordFormatterTests
{
    ...
    public void FullIntegrationTest()
    {
        ...
        Assert.Matches(@"^\d\d:\d\d:\d\d INFO \[HelloFrom\] [0-9a-f]{32}-[0-9a-f]{16}", output);

        // Check second line contains the message with parameters
        Assert.Contains($"Hello from tomato 2.99.", output, StringComparison.InvariantCulture);
    }
...
#pragma warning restore CA1848
        ...
        Assert.Contains(
            $"User Alice with ID 12345 logged in",
            output,
            StringComparison.InvariantCulture
        );
    }
    ...
    public void ExceptionLogTest()
    {
        ...
        Assert.Contains(
            $"This is a critical error with exception",
            output,
            StringComparison.InvariantCulture
        );
    ...
    public void TimestampOutputTest(bool useUtc)
    {
        ...
        var match = Regex.Match(
            output,
            @"^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d{3}) INFO \[TestLog\]"
        );
